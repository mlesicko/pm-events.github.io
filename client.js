...
document.getElementById("youtubeen").onclick = () => window.open("https://www.youtube.com/c/PokemonMastersGame");
document.getElementById("youtubejp").onclick = () => window.open("https://www.youtube.com/user/PokemonCoJp");
...
document.getElementById("rumors").onclick = () => {
  ...
  setInterval(() => {
    ...
    if (eventsCount !== events.length) {
      ...
      document.getElementById("rumorstitles").innerHTML = nextEvents.length >= 1 ? "<b style=\"\">" + nextEvents.map(e => e.name.replace(/\[grid\]/g, "<img src=\"./grid.png\"/>").replace(/\[ex\]/g, "<img src=\"./ex.png\"/>").replace(/\[gridEX\]/g, "<img src=\"./grid.png\"/><img src=\"./ex.png\"/>")).join("<br>") + "</b>" : "No rumors yet.";
...
document.getElementById("ongoing").onclick = () => {
  ...
  setInterval(() => {
    ...
    const events = rumors.filter(e => new Date(e.date).getTime() - Date.now() <= 0 && new Date(e.expires).getTime() >= Date.now()).sort((a,b) => new Date(a.expires).getTime() - new Date(b.expires).getTime());
    if (events.length === 0) {
      document.getElementById("ongoinglist").innerHTML = "No events running at the moment.";
    } else if (events[0].name !== firstEvent || events[events.length - 1].name !== lastEvent) {
      firstEvent = events[0].name;
      lastEvent = events[events.length - 1].name;
      document.getElementById("ongoinglist").innerHTML = events.map(e => e.name.replace(/\[grid\]/g, "<img src=\"./grid.png\"/>").replace(/\[ex\]/g, "<img src=\"./ex.png\"/>").replace(/\[gridEX\]/g, "<img src=\"./grid.png\"/><img src=\"./ex.png\"/>") +  ` - Ends on <span style="color:darkgrey;">${new Date(e.expires).toDateString()}</span>`).flat().join("<br>") || "No events running at the moment.";
    }
  }, 1000);
